...
try:
    ...
    raise RuntimeError(
        "\n\nalphafold is not installed. Please run `pip install kagglefold[alphafold]`\n"
    )
...
from kagglefold.alphafold.models import load_models_and_params
from kagglefold.alphafold.msa import make_fixed_size
from kagglefold.citations import write_bibtex
from kagglefold.kagglefold import chain_break, plot_paes, plot_plddts, run_mmseqs2
from kagglefold.download import default_data_dir, download_alphafold_params
from kagglefold.plot import plot_msa
from kagglefold.utils import (
...
def predict_structure(
    ...
    for (model_name, model_runner, params) in model_runner_and_params:
        ...
        prediction_result, recycles = model_runner.predict(input_features,0)
...
def run(
    ...
    print(f"Running kagglefold {version}")
...
def main():
    ...
    parser.add_argument(
        ...
        "This can make kagglefold much faster by only running the first model for easy queries.",
        type=float,
        default=100,
    )
